...
class Juego:
    ...
    def es_estado_ganador(self, estado, turno, jugador):
        """Sólo está definida para estados finales. Devuelve True
        cuando, ante la situación descrita en estado tocándole jugar a
        turno gana la partida jugador.""" 
        abstract
        
    def movimientos(self, estado):
        """Devuelve las movimientos aplicables a un estado dado. Lo
        normal es que aquí se devuelva una lista, pero si hay muchas
        se podría devolver un iterador, ya que sería más eficiente."""
        abstract

    def aplica(self, movimiento, estado):
        """ Devuelve el estado resultante de aplicar movimiento a
        estado. Se supone que movimiento es aplicable a estado (es
        decir, debe ser uno de los movimientos de
        self.movimientos(estado)."""
        abstract

    def f_utilidad(self, estado, turno):
        """Sólo está definida para estados finales. Devuelve la
        valoración del final de partida descrito en estado tocándole
        jugar a turno."""
    ...
    def f_evaluacion(self, estado, turno):
        """Devuelve la valoración de la situación descrita en el
        estado tocándole jugar a turno."""
        if self.es_estado_final(estado):
            return self.f_utilidad(estado, turno)
        else:
            return 0
...
def escribe_movimientos(juego, movimientos):
    ...
    for m in movimientos:
        print('      ({1}) {0}'.format(juego.str_movimiento(m), numero))
        numero += 1
...
def jugada_maquina(juego, procedimiento, nodo_j):
  ...
  print('Elijo: ' + juego.str_movimiento(movimiento))
